...
public class MyFileClass {
	...
	public MyFileClass(String path) { //only for directories
		this.path = path;
		this.fileHandle = new File (path);
	}
	
	public MyFileClass(String path, String fileName) {//for files with paths
	...
	//---------------------------
	public  boolean whatToDoIfFileExist(String stop_continue)
	{
	 if(fileHandle.exists() && stop_continue=="stop")
		 {System.out.println("File "+fileHandle.getName()+" already exists ...");
		 System.exit(0);
		 } 
	 return true;
	}
	//---------------------------
	public File getFileHandle () {return fileHandle;}
	//---------------------------
